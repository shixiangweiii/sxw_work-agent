...
- `npm run verify:all`: run the 17 acceptance scripts; use an individual `verify:*` command for evidence. `npm run verify:mcp` uses a fake stdio server and does not launch Playwright or access the network. `npm run verify:model-audit` uses only a loopback fake Anthropic-compatible server.
