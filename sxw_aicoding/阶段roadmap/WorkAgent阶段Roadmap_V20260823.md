...
> 当前位置：**阶段 1 已完成，待进入阶段 2（持久化与 Resume）**
...
| **阶段 1** | 主循环跑通，端点差异挡在外面 | 终端里跑真任务，进程关了就忘 | ✅ 已完成 |
...
| 项 | 结果 |
|---|---|
| 代码 | 52 个 TS 文件 / 6619 行，`tsc --noEmit` 干净 |
| Port | 14 个接口，10 个实现（与 D-14 一致） |
| 真实端点 | 百炼 Anthropic 形状跑通多轮多工具任务，具名 Terminal 退出 |
| `verify:endpoint-profile` | ✅ 换端点声明 → Runtime 行为改变，主循环文件字节未变 |
| `verify:pairing` | ✅ 四个场景 call/result 一一对应，三条中断路径全覆盖 |
| `verify:resume` | ✅ 丢弃 LoopState 后仅凭 transcript 重建并继续到终态 |
