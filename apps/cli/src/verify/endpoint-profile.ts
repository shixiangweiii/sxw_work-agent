...
async function main(): Promise<void> {
  ...
  // ── F. 判据：主循环代码指纹
  section("F. 主循环代码是否被改动");
  const after = fingerprint();
  for (const f of LOOP_FILES) {
    fact(f.replace("packages/harness-runtime/src/", ""), before[f] === after[f] ? "未改动" : "已改动");
  }

  const unchanged = LOOP_FILES.every((f) => before[f] === after[f]);
  const e2eDiffers =
  ...
  const boundaryOk = unchanged && roleFake === "PLACEHOLDER_REQUIRED" && !vFake.ok;
  verdict(
    boundaryOk,
    unchanged
      ? "端点行为是数据不是代码：换声明改变了 Runtime 行为，主循环文件字节未变"
      : "主循环文件发生了改动 —— 这不该发生",
  );

  // 【定】失败必须以非零退出码收场，否则 verify:all 的 && 链检测不到它，
  // 上面那些断言就只是打印，不是判据。
  if (!(e2eDiffers && boundaryOk && usageOk)) process.exitCode = 1;
