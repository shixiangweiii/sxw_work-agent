 ...
 * ── 为什么这条脚本必须存在 ─────────────────────────────────────────────
 *
 * 一个早期覆盖缺口是：`ScriptedModelPort` 返回写死的
  ...
  try {
    ...
    console.log(
      "   【定】A 段证明的是**读取点**能用，不是这条轴在真实 Run 里开着 ——\n" +
        ...
        "   代价是 2026-08-28 办公任务实跑的题 1 单次 run 烧掉 420,784 billed input token，\n" +
        "   一条轴都没拦住。\n",
    );
     ...
     *
     * 2026-08-28 办公任务实跑的题 1：一次 trial 在 600 秒预算里跑了 645 秒、
    ...
    console.log(
      "   【定】预算只在循环顶部查一次，两次检查之间的那个模型调用可以跑任意久 ——\n" +
        ...
        "   2026-08-28 办公任务实跑的题 1 三次分别在 645 / 809 / 1,011 秒才结算（限额 600 秒），\n" +
