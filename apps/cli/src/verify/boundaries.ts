 ...
 *
 * ══════════════════════════════════════════════════════════════════════
 * 【定】它住在单独的文件里，所有验收脚本都从这里读同一份规则：
 *
 *   · `verify:tools` A 段 —— 机械跑完整张表；
 *   · `verify:ui` / `verify:mcp` / `verify:shell` —— 取指定规则做判别力或就地复核。
 *
 ...
 * ADR-0011 新增第 12 条；2026-09-01 新增第 13 条（写死的顶栏高度）。
 *
...
export const BOUNDARIES: Boundary[] = [
  ...
  {
    /**
     * ★2026-09-01。**界面高度不得由「视口减去一个写死的常数」算出来。**
     *
     * 它与第 11 条同源：两条挡的都是**看起来能跑、实际静默出错**的布局写法，
     * 而这一类失败在截图里看不出来。
     *
     * 具体形态：`main { height: calc(100vh - 45px) }` —— 那个 45px 是顶栏高度。
     * 它只在顶栏恰好一行时成立，而顶栏一换行（接一个 MCP 服务器、列出 24 个
     * 工具名就会）`main` 就比视口高，整页多出第二条滚动条，
     * 于是「固定住的 tab 条」跟着整页一起滚 —— 而 CSS 里没有任何东西会报错。
     *
     * 【定】处置一律是**让布局自己推出高度**（flex 列：顶栏 auto ＋ main 占满剩余），
     * 不是「把 45 改成 130」。改数字只是把同一个 bug 挪到下一次文案变更。
     * 这与本仓一贯的「条数由 `BOUNDARIES.length` 推出，不写死」是同一条纪律。
     *
     * ⚠️ 射程盲区，如实写明：它只抓 `100vh`。有人写 `calc(100% - 45px)` 或
     * 用 JS 量顶栏高度再设 style，这条一个字都不会命中。不去扩模式
     * （`calc(` 在正常样式里到处都是，扩了就是噪音）——
     * 一条声称射程比实际更大的规则比一条老实交代边界的更危险。
     */
    id: "13",
