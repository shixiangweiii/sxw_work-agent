...
依据 [ADR-0012](sxw_aicoding/ADR/历史归档/0012-审批与执行特权是两条正交的档位轴.md)。
...
依据 [ADR-0011](sxw_aicoding/ADR/历史归档/0011-通过外部-MCP-接入浏览器能力.md)。
...
> 无条件报错，那条判据**照样是绿的**。与办公任务实跑 `read_blob.line_offset`
...
> `request_handoff` 请人登录（办公任务实跑 B 组的结论是它**倾向零调用**）·
...
**阶段 3（通用能力面）开发完成（2026-08-28）。** 当时按四批 S1…S14 完成。
...
**明确不做**：`CapabilityLeasePort`（决 5，见 [ADR-0005](sxw_aicoding/ADR/历史归档/0005-PARKED-lease-不做的理由.md)）、
...
>
> 详见 [ADR-0010](sxw_aicoding/ADR/历史归档/0010-二进制交付物走字节通道与按路径声明.md)。
...
> 处置见 [ADR-0012](sxw_aicoding/ADR/历史归档/0012-审批与执行特权是两条正交的档位轴.md) ——
> 不是加第三条闸门，是把审批做成一条有名字的轴。上面那句
> 「沙箱是唯一还在的闸门」在 `--sandbox off` 下**不再成立**，见下。

### 阶段 3.5 的另两个工具（同批，2026-08-30）

**`ask_user`（机制工具）—— 问用户一道选择题。** [ADR-0008](sxw_aicoding/ADR/历史归档/0008-ask-user-与-request-handoff-是两个洞.md)

> ### 【定】它与 `request_handoff` 是**两个**洞，不是一个
>
> `request-handoff.ts` 原本写着「它同时是 U-8 的落点 —— 两个洞是同一个」，
> 当时的登记也据此把 U-8 记为阶段 3 已关。**重新核对语义后：那句话对了一半。**
...
**`fetch_url` 的 `as` 参数 —— HTML 默认转 Markdown。** [ADR-0007](sxw_aicoding/ADR/历史归档/0007-html-结构转换可内置语义挑选不可.md)
...
> 而它已经被当成教训写进了复盘。**记进教训里的装饰会被抄到第二处去。**
...
三份独立评审（kimi / zocode / pi）逐条回源码复核后，**已确认成立的全部修完**。它关掉的两类：
...
### 办公任务实跑 Bugfix 批次（2026-08-28，A / A′ / B / C 四组）

当时的办公任务实测 pass@1 = 4/9、NO-GO。
...
其中一条正是当时复盘列为「最有价值的发现」的那条。手写表达式漏一项不会有任何征兆。
...
>
> ⚠️ **`turndown` 是阶段 3.5 破的基线**（原本只有 2 个，见 [ADR-0007](sxw_aicoding/ADR/历史归档/0007-html-结构转换可内置语义挑选不可.md)）。
...
> 阶段 2 起：用 `modelPortOverride` 时不再要求真凭证；现有验收均走这条路径。
...
`sxw_aicoding/` 保存研究、评审和历史设计材料。**现行行为以源码、README 与可执行验收为准**；
`历史归档/` 下的文档只解释设计来路，不再冒充当前正典。

| 文档 | 作用 |
|---|---|
| [架构设计 V05](sxw_aicoding/架构设计/历史归档/WorkAgent架构设计_V20260823_05.md) | 历史设计基线；代码注释里的 `V05 §x.y` 用它解释来路 |
| [上位基线 v0.4](sxw_aicoding/方案讨论/历史归档/WorkAgent目标定位与技术架构三次对焦讨论进展.md) | 历史目标与上位原则 |
| [阶段 1 Bugfix 批次评审](sxw_aicoding/代码评审/2026-08-25/阶段1Bugfix批次评审-zcode.md) | 仍保留的阶段 1 修复评审 |
| `代码评审/` | 按日期分目录 |
| [历史 ADR](sxw_aicoding/ADR/历史归档/) | ADR-0001…0012 与模板，保留决策背景 |
| `spikes/s0-provider-protocol/` | 一次性探针与原始证据，已完成，不进主干依赖（`tsconfig.json` 已 exclude） |

所有 `历史归档/` 内容都不再作为现行实现依据。
...
- **未接线比不写更糟**：类型、事件、类都在但运行时从不执行，会让人以为问题已经解决了；此前已有多项这种误报。要么接线，要么删掉。
