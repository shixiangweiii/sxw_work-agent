...
export async function* runLoop(
  ...
  /**
   * 模型最后说的那段话，用来填 `RunOutcome.summary`。
   *
  ...
  while (true) {
    ...
    if (verdict.kind === "SOFT" && !softLimitAnnounced.has(verdict.axis)) {
       ...
       * 一个提醒只有被提醒的对象收到才叫提醒。
       *
       * 2026-08-28 办公任务实跑的题 1：三次 trial 全部撞硬墙，
     ...
     *
     * 2026-08-28 办公任务实跑：三次 trial 分别在 645 / 809 / 1,011 秒才结算，
     ...
     * 这一行做的正是它隔了几十行明令禁止的事。
     *
     * 2026-08-28 办公任务实跑 14/14 个 run 都因此打出一条红色的假漂移
     ...
     * 办公任务实跑那条教训的形状（「这条判据要区分的两个值，在夹具里相等吗」）。
     *
