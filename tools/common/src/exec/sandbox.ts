 ...
 * 【定】**不做转义猜测：含引号或反斜杠就抛。**
 *
 * 这段注释此前挂错了地方（它写在 `realOf` 头上，被后一个 doc 块顶掉，
 * 而 `sbplString` 自己一行文档都没有），措辞也说反了 —— 原文是「必须转义」，
 * 而实现从来没有转义过任何东西，它是拒绝。
 *
 * 拒绝的理由：sbpl 的引号转义规则我们没有实测过，而猜错的方向是**放宽**
 * —— 一个被截断的 subpath 会变成一个更大的授权范围
 * （`/Users/x"y/ws` 截成 `/Users/x`），那是沙箱最不该出的错。
 * 换一个不含引号的 workspace 路径是三秒钟的事，猜错则没有任何征兆。
 * ══════════════════════════════════════════════════════════════════════
 */
